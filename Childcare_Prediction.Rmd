...
```{r}
params <- finalize(params, childcare_train)
...
best_xgb <- select_best(xgb_rs, metric = "rmse")
best_xgb

```

```{r}
show_best(xgb_rs, metric = "rmse", n = 5)
...
set.seed(123)
cv_folds <- vfold_cv(childcare_train, v = 5)
...
final_xgb_fit %>%
  extract_fit_parsnip() %>%
  vip::partial_dependence(
    pred.var = "mhi_2018",
    train = bake(prep(xgb_rec), new_data = childcare_train)
  )
...
final_xgb_fit %>%
  extract_fit_parsnip() %>%
  vip::partial_dependence(
    pred.var = c("mhi_2018", "flfpr_20to64"),
    train = bake(prep(xgb_rec), new_data = childcare_train),
    plot = TRUE,
    contour = TRUE
  )
