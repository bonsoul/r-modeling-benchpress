...
summary_table <- df %>%
  group_by(whohivdiseasestage, hivstatusbeforepregnancy) %>%
  summarise(n = n(), .groups = "drop") %>%
  group_by(whohivdiseasestage) %>%
  mutate(percent = round(100 * n / sum(n), 2)) %>%
  ungroup()

# Create gt table
summary_table %>%
  gt(groupname_col = "whohivdiseasestage") %>%
  fmt_number(
  ...
  tab_header(
    title = "Diagnosis Timing by WHO HIV Disease Stage"
  )
