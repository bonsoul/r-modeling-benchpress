...
```{r}
all_preds <- bind_rows(
  knn_res %>% unnest(.predictions) %>% mutate(model = "kknn"),
  tree_res %>% unnest(.predictions) %>% mutate(model = "rpart")
)
...
ggplot(roc_data,
  ...
  theme_minimal() +
  labs(title = "ROC Curves: KNN vs Decision Tree")
...
folds_df <- tibble(
  row = 1:nrow(hotel_train),
  fold = rep(1:25, length.out = nrow(hotel_train))
)

# Plot the fold assignments
ggplot(folds_df, aes(x = row, y = fold, color = factor(fold))) +
  geom_point(size = 1) +
  labs(title = "25-Fold Cross-Validation Assignment",
       x = "Observation Index",
       y = "Fold",
       color = "Fold") +
  theme_minimal()
