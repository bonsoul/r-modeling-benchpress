...
daf2 <- daf %>%
  mutate(
    positive_symptoms =
      rowSums(across(all_of(phq_items_daf[1:8]), ~ .x >= 2), na.rm = TRUE) +
      ifelse(PHQ_item_10 >= 1, 1, 0)
  )
...
```{r}
table(daf3$positive_symptoms)
table(daf3$MDD_PHQA_screen)
...
severity_table %>%
  rename(`Count (N = 122)` = count) %>%
  kable(
    caption = "Depression Severity Based on PHQ-9 Total Scores",
    align = c("l", "r", "r"),
    format = "html"
  ) %>%
  kable_styling(
...
depressed_table %>%
  rename(`Count (N = 122)` = count) %>%
  kable(
    caption = "Depression Status Distribution (N = 122)",
    align = c("l", "r", "r"),
    format = "html"
  ) %>%
  kable_styling(
