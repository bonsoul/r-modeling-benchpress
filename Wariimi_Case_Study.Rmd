...
adherence_vars <- c(
  "do_you_ever_miss_the_medicines_you_are_supposed_to_take",
  "do_you_ever_refuse_to_take_the_medicines_you_are_supposed_to_take",
  "do_you_ever_not_take_the_medicines_because_you_do_not_want_to_take_them_in_front_of_other_people",
  "do_you_ever_have_problems_taking_the_medicines_because_you_do_not_know_why_you_are_taking_them",
  "do_you_ever_have_problems_taking_the_medicines_on_time_or_taking_them_every_day"
)

# Loop through variables and print frequency tables
for (var in adherence_vars) {
  cat("\nVariable:", var, "\n")
  freq <- table(dfs_merged[[var]])
  percent <- round(prop.table(freq) * 100, 1)
  print(cbind(Frequency = freq, Percentage = percent))
}
...
```{r}
d <- dfs_merged %>%
  select(all_of(adherence_vars)) %>%
  mutate(across(
    everything(),
    ~ gsub("^[a-d]\\)\\s*", "", .x) %>%        # remove "a) ", "b) ", etc.
      factor(levels = c("Never", "Once", "Sometimes", "Severally"), ordered = TRUE)
  ))

```

```{r}
lapply(dfs_merged[adherence_vars], unique)

```

```{r}
labels_list <- list(
  do_you_ever_miss_the_medicines_you_are_supposed_to_take = "Miss medicines",
  do_you_ever_refuse_to_take_the_medicines_you_are_supposed_to_take = "Refuse medicines",
  do_you_ever_not_take_the_medicines_because_you_do_not_want_to_take_them_in_front_of_other_people = "Avoid taking meds in front of others",
  do_you_ever_have_problems_taking_the_medicines_because_you_do_not_know_why_you_are_taking_them = "Problems understanding why taking meds",
  do_you_ever_have_problems_taking_the_medicines_on_time_or_taking_them_every_day = "Problems taking meds on time/daily"
)
...
```{r}
dfs_merged$adherence_binary <- factor(
  dfs_merged$adherence_binary,
  levels = c("Adherent", "Non-adherent"),
  labels = c("Good adherence", "Poor adherence")
)
...
dfs_merged %>%
  ...
  tbl_summary(
    ...
    missing = "no",
    label = list(
      adherence_binary ~ "Medication adherence"
    )
  )
...
```{r}
table(dfs_merged$adherence_binary)
...
# Use a named vector for recoding
labels_vector <- c(
  "do_you_ever_miss_the_medicines_you_are_supposed_to_take" = "Miss medicines",
  "do_you_ever_refuse_to_take_the_medicines_you_are_supposed_to_take" = "Refuse medicines",
  "do_you_ever_not_take_the_medicines_because_you_do_not_want_to_take_them_in_front_of_other_people" = "Avoid taking meds in front of others",
...
# Recode using the named vector
summary_df <- summary_df %>%
  mutate(
    Variable = labels_vector[Variable]  # safe recoding
  )

# View nicely in RStudio Viewer
summary_df %>%
  gt() %>%
  tab_header(title = "Adherence Variable Frequencies")
...
all_tables <- lapply(adherence_vars, function(var) {
  freq <- table(dfs_merged[[var]])
  percent <- round(prop.table(freq) * 100, 1)
  df <- data.frame(
    Variable = var,
...
# Combine all
all_tables_df <- do.call(rbind, all_tables)
...
# Loop through adherence variables
for (var in adherence_vars) {
  df <- dfs_merged %>%
    count(!!sym(var)) %>%                        # count frequencies
    mutate(
...
for (var in unique(summary_df$Variable)) {
  cat("\n", var, "\n")
  tmp <- summary_df %>% filter(Variable == var) %>% select(-Variable)
  print(tmp)
}
...
# Combine all adherence variables into one table
all_tables <- lapply(adherence_vars, function(var) {
  dfs_merged %>%
    count(!!sym(var)) %>%
    mutate(
...
summary_df <- bind_rows(all_tables)
...
```{r}
# Select adherence variables and convert to ordered factors
d <- dfs_merged %>%
  select(all_of(adherence_vars)) %>%
  sjmisc::to_factor(ordered = TRUE)

# View frequencies and percentages
view_df(d, show.frq = TRUE, show.prc = TRUE)

# Plot Likert-style stacked bars
...
```{r}
tbl_word <- tbl %>% as_flex_table()

doc <- read_docx() %>%
  body_add_par("Adherence to ART Medication", style = "heading 1") %>%
  body_add_flextable(tbl_word)

print(doc, target = "Adherence_Table.docx")

```

```{r}
dfs_merged[adherence_vars] <- lapply(dfs_merged[adherence_vars], function(x) {
...
```{r}
dfs_merged$non_adherence_score <- rowSums(
  dfs_merged[adherence_vars],
  na.rm = TRUE
)
...
```{r}
summary(dfs_merged$non_adherence_score)
hist(dfs_merged$non_adherence_score)
