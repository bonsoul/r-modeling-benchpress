...
# Convert adherence variables to ordered factors with descriptive labels
d <- dfs_merged %>%
...
dfs_merged <- dfs_merged %>%
  mutate(
    adherence_binary = case_when(
      rowSums(across(all_of(adherence_vars),
                     ~ .x %in% c("Sometimes", "Often")),
              na.rm = TRUE) == 0 ~ "Adherent",
      TRUE ~ "Non-adherent"
    )
  )
...
```{r}
table(dfs_merged$adherence_binary1)
prop.table(table(dfs_merged$adherence_binary1)) * 100
