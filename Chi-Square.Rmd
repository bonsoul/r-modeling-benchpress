...
```{r}
severe_data <- merged_data %>%
  dplyr::filter(PHQ_severity %in% c(
    "Moderate",
    "Moderately Severe",
    "Severe"
  ))
...
```{r}
severe_data$PHQ_severity <- droplevels(severe_data$PHQ_severity)

```

```{r}
test_results <- lapply(adherence_vars, function(var) {
  
  tab <- table(severe_data$PHQ_severity, severe_data[[var]])
  ...
  test <- if (use_fisher) fisher.test(tab) else chi
  
  data.frame(
    Variable = var,
    Test_Used = ifelse(use_fisher, "Fisher's Exact", "Chi-square"),
    Statistic = ifelse(use_fisher, NA, round(test$statistic, 2)),
    df = ifelse(use_fisher, NA, test$parameter),
...
final_results_neat <- final_results %>%
  mutate(
...
kable(
  final_results_neat,
  caption = "Association Between PHQ Depression Severity and Medication Adherence Behaviors",
  col.names = c("Adherence Variable", "Statistical Test", "Test Statistic", "df", "p-value"),
  align = "lcccc"
)
...
```{r}
table(severe_data$PHQ_group, severe_data$Miss_Medicines)
fisher.test(table(severe_data$PHQ_group, severe_data$Miss_Medicines))
